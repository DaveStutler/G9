...
In our project, it makes perfect sense to use a 5-layer ANN to handle complex data. The linear activation function is one of the simplest activation functions. It only performs a linear combination of inputs and weights, so calculations are very fast, speeding up the entire model. The Leaky ReLU activation function helps the gradient by solving the "neuron death" problem by setting a small slope on the negative input (usually a small constant, such as 0.01) so that the gradient does not disappear completely in these regions Propagate and train deeper neuron networks. Its output is not strictly zero in the negative region, which helps the network learn and converge, and this activation function can fit the data better in some cases because it has non-zero values over a wider range of input gradients value. To sum up, choosing the appropriate activation function and number of network layers can improve the performance of the model.

* Data Preprocessing: It is common and standard practice to split the data set into 90:10 training and test sets to evaluate the performance of the model.
Performance metrics: This experiment uses mean squared error (MSE) and accuracy to evaluate the model's performance. These two metrics provide important information about the model's performance on training and test data. MSE is used to measure the prediction error of continuous output, while accuracy is used to evaluate the performance of classification models. Using these two metrics is appropriate because they provide information about different aspects of performance.

* Tuning and improvements: While some description of the model is provided, there is no mention of hyperparameter tuning or other steps for further improvement in the lab report. In practical applications, multiple attempts and adjustments are usually required to optimize model performance.

* For analysis of the results, we wrote code to evaluate the performance of the neural network model on the test data, including MSE loss values as well as early stopping rounds and best-performing model saving rounds. These metrics help understand the model's generalization ability and the training process. The specific analysis is as follows: We stopped after epoch=11
Best model saved after epoch=2, because the early stopping parameter is set to stop training when there is no performance improvement within 8 consecutive epochs.
We stopped when the validation mse was: 0.011218510568141937
"MSE testing is:0.006146725842076111.

This is theoretically possible for Kfolds analysis, but currently it requires too many GPUs, and furthermore, we don't think it's necessary since the mean square error we get from the original model is already low. We decided to include it as a review anyway. The purpose of this code is to demonstrate a possible cross-validation method, but in real-world situations it may require more computing resources to run.

For the MSE results, the average MSE of the model on 5 different random folds is 0.011, with a standard deviation of 0.000632455532033676. The average MSE can be used to measure the overall performance of the model, and the standard deviation is used to indicate the dispersion of these scores. A smaller standard deviation generally indicates smaller differences in model performance across folds.

Data Fitting: Check out our collaboration document
